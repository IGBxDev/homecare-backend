    ...
    async getCNPJ(cnpj) {
        ...
        try {
            ...
            if (!validacnpj.test(cnpj)) {
                throw new CustomError_1.CustomError('Formato de CNPJ inválido', 404);
            }
            const response = await instance.get(url);
            const cnpjReponse = response.data;
            if ((_a = response.data) === null || _a === void 0 ? void 0 : _a.erro) {
                throw new CustomError_1.CustomError('CNPJ não encontrado', 404);
            }
            if (response.status == 429) {
                throw new CustomError_1.CustomError('Muitas requisições', 429);
            }
            if (response.status == 504) {
                throw new CustomError_1.CustomError('Timeout', 504);
            }
            return cnpjReponse;
        }
        catch (error) {
